...
--vancouver
select * from [dbo].[df_toronto_availability]
select * from[dbo].[host_toronto_df]
Select * from [dbo].[listing_toronto_df]
Select * from [dbo].[review_toronto_df]
...
Select Month,host_is_superhost,Avg(Total_Bookings)Over(Partition by Month,host_is_superhost Order by Month) as Avg_bookings 
from(
...
Select A.Property_type,Count(A.Host_ID) as Total_Property_Other_Host
From listing_vancouver_df A Inner Join host_vancouver_df B 
...
Select A.Host_id, Avg(host_response_rate) As host_response_rate,Avg(host_acceptance_rate) 
as host_acceptance_rate from [dbo].[host_vancouver_df] B Inner Join #aaa A ON A.host_id = B.host_id
where  host_response_rate is not null or host_acceptance_rate is not null GROUP BY A.host_id;
